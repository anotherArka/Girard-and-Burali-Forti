...
properties hold for an element in a type assuming they hold for elements less than it. It can
be proven that accessibility is a mere proposition. 
...
A simulation is essentially a function from one ordinal to another resepecting the order. 
Using well-founded induction we can prove that each simulation is injective. Because 
simulations are functions from one set to another and $a<b$ is a mere proposition, 
injectivity implies that being a simulation is also mere proposition. Using induction on 
$<$ we can prove that given for well-founded and extensional $(A,<)$ and $(B,<)$, there is
at most one simulation $f:A \to B$. We write $A \leq B$ to mean there exists a simulation
$A \to B$. Then $A \leq B$ is a mere proposition. Moreover it is a partial order, i.e 
...
If there is a universe $\U$ containing everything then 
