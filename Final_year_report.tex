...
The idea definitional equality is close to $\alpha$ and $\beta$ reductions in logic. For 
example, in case of natural numbers $1 + 0$ and $0 + 1$ are equal by definition of $+$. But
the statement that for any $n$, $n + 0$ and $0 + n$ is a proposition which needs to be 
proven.\smallskip
...
the function type $A \to B$, the product type $A \times B$ etc. Unlike set theory functions
and products are first-class objects in type theory. Although their introduction and 
elimination rules agree with usual notions. 
 

\subsection{Proposition as Types}

\subsection{Universes and Univalnece}
