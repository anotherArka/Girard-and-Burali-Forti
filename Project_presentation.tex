...
\only<1->{
\begin{block}{}
In homotopy type theory(HOTT) types are interpreted as spaces. This way each statement in
HOTT gets a topological interpretation. 
\end{block}
}

\only<2->{
\begin{block}{Equality Types}
The proposition that two elements of the 
same type $a, b : A$ are equal is considered as a type $a =_A b$. 
...
\only<3->{ 
\begin{block}{Maybe say this in words rather than in slide}
Topologically $a =_A b$ can be interpreted as the space of paths from $a$ to $b$. Then 
${refl}$ can be considered as the subspace of constant paths/loops. The rule that equality
